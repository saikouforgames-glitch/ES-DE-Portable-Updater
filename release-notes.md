## ES-DE Portable Updater v1.2.1

### Fixes

- **ES-DE inside subfolders of protected locations is now accepted** — installations kept at paths like `C:\Users\<user>\Downloads\ES-DE` or `C:\Program Files\ES-DE` previously reported "system directory" and were refused. Only the protected locations themselves (drive roots, `C:\Windows`, Program Files (x86), ProgramData, the user profile root, `C:\Users`, `$Recycle.Bin`) remain refused. All existing safety checks still apply (structural validation, physical identity checks, update preview, running-program guard, folder seal).
